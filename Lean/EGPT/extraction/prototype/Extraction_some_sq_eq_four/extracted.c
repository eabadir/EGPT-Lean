...
/* Phase 3a-prototype — term-content extraction of Extraction.some_sq_eq_four */
EgptNumber *Extraction_some_sq_eq_four(void) {
    return ({ EgptNumber *__result = egpt_from_i64(0); for (int __i = 0; __i < 16; ++__i) { EgptNumber *__witness = egpt_from_i64((int64_t)__i); int __hold = (egpt_cmp(egpt_mul(__witness, __witness), egpt_from_i64(4)) == 0); if (__hold) { egpt_free(__result); __result = __witness; break; } egpt_free(__witness); } __result; });
}

int main(void) {
    EgptNumber *r = Extraction_some_sq_eq_four();
