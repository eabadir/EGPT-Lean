...
/* Target theorem: Extraction.exp_one_between_two_and_three */
int main(void) {
    int result = (((egpt_cmp(egpt_from_i64(2), exp_partial_sum(egpt_from_i64(1), BUDGET)) == -1)) && ((egpt_cmp(exp_partial_sum(egpt_from_i64(1), BUDGET), egpt_from_i64(3)) == -1)));
    printf("%s\n", result ? "true" : "false");
    return result ? 0 : 1;
}
