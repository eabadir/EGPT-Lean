...
/* Target theorem: Extraction.two_lt_one_plus_sqrt_two */
int main(void) {
    int result = (egpt_cmp(egpt_from_i64(2), egpt_add(egpt_from_i64(1), sqrt_partial_newton(egpt_from_i64(2), BUDGET))) == -1);
    printf("%s\n", result ? "true" : "false");
    return result ? 0 : 1;
}
