...
/* Target theorem: Extraction.exp_one_lt_three_or_huge */
int main(void) {
    int result = (((egpt_cmp(exp_partial_sum(egpt_from_i64(1), BUDGET), egpt_from_i64(3)) == -1)) || ((egpt_cmp(egpt_from_i64(100), exp_partial_sum(egpt_from_i64(1), BUDGET)) == -1)));
    printf("%s\n", result ? "true" : "false");
    return result ? 0 : 1;
}
