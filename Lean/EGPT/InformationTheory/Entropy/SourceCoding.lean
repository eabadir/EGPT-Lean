...
public import InformationTheory.Entropy.Program
...
/-- A choice-free primality-like test for powers of two. -/
def isPowerOfTwo_C (n : ℕ) : Bool :=
  if n = 0 then false else (n.land (n - 1)) = 0

/-- A computable formulation of the ceiling of the Shannon entropy of a `FiniteIIDSample`.
    Using $H_{total} = \log_2 (A / B)$ where $A = (p+q)^{m(p+q)}$ and $B = p^{mp} q^{mq}$,
    we find the smallest integer $L$ such that $B \cdot 2^L \ge A$. -/
def computableEntropyCeil (p q m : ℕ) : ℕ :=
  if p = 0 then 0
  else if q = 0 then 0
  else
    let A := (p + q) ^ (m * (p + q))
    ...
    let C := (A + B - 1) / B
    if C = 0 then 0
    else if isPowerOfTwo_C C then Nat.log2 C
    else Nat.log2 C + 1

/-- **The Direct Formal Hook:** The optimal source-code for any information source
is literally an `EntropyNat` bit-tape. This explicitly connects Shannon Source Coding
to the EGPT number-system bijection. 

This hook is fully computable and mathematically choice-free, operating entirely
in the integer domain (`computableEntropyCeil`). -/
def sourceToEntropyNat (src : InformationSource) : EntropyNat :=
  EntropyNat.ofNat (computableEntropyCeil src.p_param src.q_param src.num_sub_samples)
