...
public import InformationTheory.Entropy.Concrete
public import InformationTheory.Entropy.SourceCoding
public import InformationTheory.EntropyNumber.Factorization
public import InformationTheory.EntropyNumber.Hierarchy
public import InformationTheory.EntropyNumber.LogarithmBijective
...
/-- Map a `List ℕ` of primes into a `List PNat` by filtering positive elements. -/
def primesToPNat (L : List ℕ) : List PNat :=
  L.filterMap (fun x => if h : x > 0 then some ⟨x, h⟩ else none)

/-- The choice-free Fundamental Theorem of Arithmetic via Information.
    The total information of `n` equals the sum of the information of its prime factors (with multiplicity).
    The factors are folded using the natively additive `log_add` operator on `ConstructibleLog`. -/
theorem fta_via_information_C (n : ℕ) (hn : n > 0)
    (h_prod : (primesToPNat (factorListC n)).prod = ⟨n, hn⟩) :
    log_add_list (primesToPNat (factorListC n)) = logEquivPNat.invFun ⟨n, hn⟩ := by
  rw [log_add_list_eq_prod, h_prod]
